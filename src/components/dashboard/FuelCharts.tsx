...
function groupFuelByVehicle(data: FuelRow[]) {
  const grouped = data.reduce<Record<string, number>>((acc, row) => {
    acc[row.vehicle] = (acc[row.vehicle] || 0) + row.fuel_liters
    return acc
  }, {})

  return Object.entries(grouped)
    .map(([vehicle, fuel]) => ({ vehicle, fuel }))
    .sort((a, b) => b.fuel - a.fuel)
}

function groupCostByVehicle(data: FuelRow[]) {
  const grouped = data.reduce<Record<string, number>>((acc, row) => {
    acc[row.vehicle] = (acc[row.vehicle] || 0) + row.fuel_cost
    return acc
  }, {})
  ...
  const topVehicles = sorted.slice(0, 5)
  const otherVehicles = sorted.slice(5)

  const othersCost = otherVehicles.reduce((acc, item) => acc + item.cost, 0)
...
export function FuelByVehicleChart({ data }: FuelChartProps) {
  const chartData = groupFuelByVehicle(data)
...
export function CostByVehicleChart({ data }: FuelChartProps) {
  const chartData = groupCostByVehicle(data)
